...
type EditableUser = { id: string; name: string; email: string };

// Mirror the server-side update rules (server/src/routes/users.ts). Zod
// trims/normalises as it validates, so the parsed output is what we send.
...
async function updateUser(id: string, input: EditUserInput): Promise<void> {
  try {
    await axios.patch(`/api/users/${id}`, input, { withCredentials: true });
  } catch (e) {
...
export default function EditUserModal({
  ...
  const mutation = useMutation({
    mutationFn: (input: EditUserInput) => updateUser(user.id, input),
    onSuccess: async () => {
      // Refresh the list so the edited row reflects the change, then dismiss.
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const result = editUserSchema.safeParse({ name, email });
    if (!result.success) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="mt-1 text-sm text-gray-600">Update this account's name and email.</p>
        ...
        <form onSubmit={handleSubmit} noValidate className="mt-4 space-y-4">
          ...
          {mutation.isError && (
            <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">
              Failed to update user: {mutation.error.message}
            </p>
          )}
